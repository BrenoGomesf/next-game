...
export const NavBar = ({ className, user, ...props }: NavBarProps) => {
  return (
    <nav
      className={cn(
        "fixed top-0 left-0  flex flex-col bg-slate-900 border-r border-indigo-400/20 h-screen hover:border-indigo-400/20 w-72 p-2 text-slate-300",
        `${className}`
      )}
      {...props}>
        ...
        {user ? (
          <>
            <NavBarListItemLink href={"/user"}>
              <RouteIcon className="w-4 h-4" /> {user.name}
            </NavBarListItemLink>
            <NavBarListItem>
              <FormButton action={handleSignOutForm}>
                <ExitIcon   className="w-4 h-4" />  Walkthoughs
              </FormButton>
            </NavBarListItem>
          </>
        ) : (
          <NavBarListItemLink href={"/auth/sign-in"}>
            <RouteIcon className="w-4 h-4" /> Login
          </NavBarListItemLink>
        )}
      </NavBarList>
      <NavBarList>
        <NavBarListItemLink href={"/user"}>
          <FaceHappyIcon className="w-4 h-4" />
          User
        </NavBarListItemLink>
      </NavBarList>
    </nav>
