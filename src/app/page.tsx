...
import { HomeLatestArticles } from "@/sections";
import { HomeLatestArticlesSkeleton } from "@/sections/HomeLatestArticles/HomeLatestArticlesSkeleton";
...
export default async function Home({searchParams}:{ searchParams?: { page?: string, limit?: string }}) {
  ...
  return (
    <PageWrapper>
      ...
      <h2 className="text-2xl my-6 underline"> Articles</h2>
                ...
                return (
                  <div
                    ...
                    <div className="w-full flex flex-col gap-2 pl-4">
                      ...
                      <Link href={`/articles/${article.slug}`} className="bg-slate-700 hover:bg-indigo-400/40 rounded-lg px-4 py-2 inline max-w-max">
                        Ler mais
                      </Link >
                    </div>
                  </div>
